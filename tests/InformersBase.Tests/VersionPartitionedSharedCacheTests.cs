...
namespace Steeltoe.Informers.InformersBase.Tests
{
    ...
    {
        private readonly VersionPartitionedSharedCache<string, TestResource, int> _sut;

        public VersionPartitionedSharedCacheTests()
        {
            _sut = new VersionPartitionedSharedCache<string, TestResource, int>(x => x.Key, x => x.Version);
        }
