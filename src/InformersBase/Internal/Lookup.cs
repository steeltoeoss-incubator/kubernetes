...
{
    internal class Lookup<TKey, TElementKey, TElement> : ILookup<TKey, KeyValuePair<TElementKey,TElement>>//, IAsyncIListProvider<IAsyncGrouping<TKey, KeyValuePair<TElementKey,TElement>>>
    {
        private readonly IEqualityComparer<TKey> _comparer;
        private Grouping<TKey, TElementKey, TElement>[] _groupings;
        private Grouping<TKey, TElementKey, TElement>? _lastGrouping;

        private Lookup(IEqualityComparer<TKey>? comparer)
        {
            _comparer = comparer ?? EqualityComparer<TKey>.Default;
            _groupings = new Grouping<TKey, TElementKey, TElement>[7];
        }

        public int Count { get; private set; }

        public IEnumerable<KeyValuePair<TElementKey, TElement>> this[TKey key]
        {
            get
            {
                ...
                return Array.Empty<KeyValuePair<TElementKey, TElement>>();
            }
        }
        ...
        public IEnumerator<IGrouping<TKey, KeyValuePair<TElementKey, TElement>>> GetEnumerator()
        {
            var g = _lastGrouping;
            if (g != null)
            {
        ...
        // }
        //
        // internal static async Task<Lookup<TKey, KeyValuePair<TElementKey, TElement>>> CreateAsync<TSource>(IAsyncEnumerable<TSource> source, Func<TSource, TKey> keySelector, Func<TSource, TElement> elementSelector, IEqualityComparer<TKey>? comparer, CancellationToken cancellationToken)
        // {
        ...
        // }
        //
        // internal static async Task<Lookup<TKey, TElementKey, TElement>> CreateAsync(IAsyncEnumerable<TElement> source, Func<TElement, TKey> keySelector, IEqualityComparer<TKey>? comparer, CancellationToken cancellationToken)
        // {
        //     var lookup = new Lookup<TKey, TElementKey, TElement>(comparer);
        //
        //     await foreach (var item in source.WithCancellation(cancellationToken).ConfigureAwait(false))
        //     {
        //         var key = keySelector(item);
        ...
        internal static async Task<Lookup<TKey, TElementKey, TElement>> CreateForJoinAsync(
            IAsyncInformable<TElementKey, TElement> source,
        ...
        internal Grouping<TKey, TElementKey, TElement>? GetGrouping(TKey key)
        {
        ...
        internal Grouping<TKey, TElementKey, TElement>? GetGrouping(TKey key, int hashCode)
        {
        ...
        internal Grouping<TKey, TElementKey, TElement> GetOrCreateGrouping(TKey key)
        {
            var hashCode = InternalGetHashCode(key);
            ...
            var g = new Grouping<TKey, TElementKey, TElement>(key, hashCode, new KeyValuePair<TElementKey, TElement>[1], _groupings[index]);
            _groupings[index] = g;
        ...
        private void Resize()
        {
            ...
            var newGroupings = new Grouping<TKey, TElementKey, TElement>[newSize];
            var g = _lastGrouping;
            do
            {
                g = g!._next;
        ...
        IAsyncEnumerator<IAsyncGrouping<TKey, KeyValuePair<TElementKey, TElement>>> IAsyncEnumerable<IAsyncGrouping<TKey, KeyValuePair<TElementKey, TElement>>>.GetAsyncEnumerator(CancellationToken cancellationToken)
        {
            cancellationToken.ThrowIfCancellationRequested(); // NB: [LDM-2018-11-28] Equivalent to async iterator behavior.

            return Enumerable.Cast<IAsyncGrouping<TKey, KeyValuePair<TElementKey, TElement>>>(this).ToAsyncEnumerable().GetAsyncEnumerator(cancellationToken);
        }

        ValueTask<List<IInformGrouping<TKey, TElementKey, TElement>> IAsyncIListProvider<IAsyncGrouping<TKey, KeyValuePair<TElementKey, TElement>>>.ToListAsync(CancellationToken cancellationToken)
        {
